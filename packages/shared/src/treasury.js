export const treasuryPlaybooks = [
    {
        id: "payroll",
...
export const computePrivacyPosture = (operations) => {
    ...
    let score = 42;
    if (totalMoves > 0)
        score += Math.min(28, confirmedMoves * 8);
    if (shieldedMoves > unshieldedMoves)
        score += 12;
    if (failedMoves > 0)
        score -= failedMoves * 10;
    if (operations.some((op) => op.type === "register" && op.status === "callback_confirmed"))
        score += 8;
...
export const buildComplianceReport = (operations, auditSummary) => {
    ...
    const lines = [
        ...
        "",
        "## Ledger snapshot",
        ...operations.map((op) => `- ${op.createdAt} | ${op.type} | ${op.status} | correlation ${op.correlationId}${op.failureReason ? ` | failure ${op.failureReason}` : ""}`),
        "",
        "## Recommendations",
